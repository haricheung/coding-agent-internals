...
| | 10:40 - 10:50 | | **课间休息** |
| | 10:50 - 11:20 | 修 Bug 实战剖析：多轮纠错与防死循环 | **主线问题：Agent 收到报错信息后，如何决定下一步行动？**<br>〔OODA 定位：**Orient 环节（从失败中学习）** / 完整走一遍 L→R→V 循环〕<br>1. 全流程演示：一个真实的修 Bug 流程，逐步展示 L→R→V 三阶段在 ReAct 循环中的动态执行<br>2. 双层循环模型：内层 TAO 循环 + 外层经验积累循环；失败信号（负反馈）经 Reflection 转化为结构化经验总结（正向指导），注入下一轮 Thought〔Reflexion：化失败为经验〕<br>3. Reflexion 的最小实例（来自 Demo 2 的铺垫）：格式解析失败 → harness 构造 is_error 反馈 → 模型修正格式 → 重试成功。CC 源码实证：query.ts 的 12 个退出点 + 7 个继续点，包括 max_output_tokens 恢复（注入 "Resume directly" 消息重试 3 次）和 reactive compact 紧急压缩——**harness 的工程复杂度远超"一个 while 循环"**<br>4. 循环失灵与恢复机制：Orient 环节停滞时表现为重复执行相同错误命令；工程上通过 max retries、上下文压缩、prompt 指令三重手段强制恢复循环<br>5. 对比分析：一次成功纠错 vs 一次陷入死循环的完整日志，定位关键分叉点<br>**〔Demo 3 · Live〕** 用 MVP + Qwen2.5-Coder-7B 现场修复 buggy_code.py（off-by-one bug）；每步暂停标注 L/R/V 阶段；若模型出错则当场讲解"这是 OODA 哪个环节失灵"——Harness 的兜底与 Reflexion 的雏形 |
