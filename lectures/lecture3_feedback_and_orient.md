...
〔**Demo 3 · Live**〕用 MVP + Qwen3-30B-A3B 修复 `buggy_calc.py`——一个成绩统计函数，包含两个 bug。
...
1. **模型推理能力不足**
   Orient 本身质量差——模型看到了 `Expected 80.0, got 60.0` 但无法推理出"是 >= 和 > 的边界问题"。这是小模型（如 3B 激活参数）的常见瓶颈。
...
〔**Demo 3 后半 · Live**〕如果小模型在修 Bug 过程中出错——不要重启。当场标注"这是 Orient 失灵"，指出模型的 Thought 缺少了什么信息，或者做了什么错误推理。这就是小模型作为教学工具的价值：它的失败模式清晰可见、可解释。
