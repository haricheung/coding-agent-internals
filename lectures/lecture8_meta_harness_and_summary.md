...
**模型越弱，harness 优化的相对收益越大。** 这和我们整个课程用小模型做 Demo 时观察到的现象一致——小模型对 system prompt 措辞、工具设计、上下文管理的敏感度远高于大模型。大模型有足够的智能"自救"（绕过不完美的 harness），小模型没有这个余量——harness 的质量直接决定了它的表现上限。
